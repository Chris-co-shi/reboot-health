...
import { listHealthConstraints } from '@/services/m2aApi';
import {
  cancelPlanVersion,
  confirmPlanVersion,
  getPlanVersion,
...
import type { HealthConstraint } from '@/types/m2a';
import type { PlanItem, PlanVersion } from '@/types/m2b';
...
const activeConstraints = ref<HealthConstraint[]>([]);
const cancelReason = ref('');
const cancelDialogVisible = ref(false);

const confirmAction = useIdempotentAction((key: string, id: string) => confirmPlanVersion(id, key));
const cancelAction = useIdempotentAction((key: string, id: string, reason: string) =>
  cancelPlanVersion(id, { cancelReason: reason }, key),
);

const canConfirm = computed(() => version.value?.status === 'DRAFT');
...
async function load() {
  ...
  try {
    ...
    activeConstraints.value = await listHealthConstraints({ status: 'ACTIVE', includeArchived: false });
  } catch (error) {
    showError(error);
  } finally {
    loading.value = false;
...
async function confirmVersion() {
  ...
  try {
    ...
    version.value = await confirmAction.run(version.value.id);
...
async function submitCancel() {
  ...
  try {
    version.value = await cancelAction.run(version.value.id, cancelReason.value.trim());
...
<template>
  <section v-loading="loading" class="plan-stack">
    ...
    <section class="panel">
      ...
      <h3>有效健康约束</h3>
      <el-tag v-for="constraint in activeConstraints" :key="constraint.id" class="context-tag" type="warning" effect="plain">
        {{ constraint.title }}
      </el-tag>
      <p v-if="activeConstraints.length === 0" class="muted-text">暂无有效健康约束。</p>
    </section>
