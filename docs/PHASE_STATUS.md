...
## 13. Phase 3B Slice 2 — Identity Administrative Use Cases and Token Security（2026-07-12）

- 管理员 Application 用例已完成：角色授予/撤销、账号禁用、单 Session 与全 Session 撤销均强制 `ADMIN_OPERATOR + MFA`、禁止管理自身，普通用户、服务主体和后台主体默认拒绝；未新增未批准的管理员 HTTP API。
- `permission_version` 仅在真实角色/禁用变化时增加；Access Token 始终以 PostgreSQL User、Session、Grant 与版本为权威，Redis 陈旧或故障不会绕过校验。
- Session 撤销级联 Access Grant 和 Token Family，并与 Audit/Outbox 同事务；重复撤销无重复副作用，单设备不影响其他设备。
- Refresh 重放处置现在先提交 `REPLAY_COMPROMISED`、Session/Grant 撤销和 Audit/Outbox，再向调用方返回稳定错误；PostgreSQL 17.10、12 路并发实测仅 1 个 replacement，最终有效 Access Grant 0。
- 验证阶段结果：非 PG **44 passed**，PG **15 passed**；管理员事务 Outbox 注入失败时角色、权限版本、Audit/Outbox 全回滚。无 Migration，Head 仍为 `20260712_0003`。
- 未完成：管理员 HTTP 合同、MFA disable/reset、固定安全问题、Client Credentials、完整限流、SMTP Processor、OTel。Slice 2 与 Phase 3B 保持 `IN_PROGRESS`。
