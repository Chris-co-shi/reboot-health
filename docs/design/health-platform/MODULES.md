...
- Identity 管理能力位于 Application 层并重复执行 Principal/管理员 MFA Policy；Session/Grant/Token Family 撤销、权限版本变更、Audit 与 Outbox 保持单事务。Redis 不是认证事实源，Refresh 并发由 PostgreSQL Family 行锁串行化。
