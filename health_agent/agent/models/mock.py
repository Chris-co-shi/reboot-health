...
该 Provider 用固定规则生成 INITIAL_PLANNING 草案，用于本地开发和测试。它不调用
真实模型、不访问网络，也不产生已确认事实；所有输出仍然是候选和待确认草案。
"""
...
from agent.schemas.planning import PLANNING_SCHEMA_VERSION


class MockProvider(BaseModelProvider):
    """默认测试 Provider，保证输入相同则输出稳定。"""
    ...
    def generate_initial_planning(
        ...
        """生成保守的 INITIAL_PLANNING 草案。

        Prompt 在 Mock 中不参与推理，但保留参数可以确保 MockProvider 与真实
        Provider 使用同一个接口。
        """
        del prompt
...
def _understanding_candidates(
    ...
    这些候选只是对自然语言和档案的可审计摘要，不会被自动写入长期记忆。
    """
...
def _weekly_plan_draft() -> dict[str, Any]:
    ...
    该草案刻意保守：强调 RPE、症状停止规则和复盘，不安排高强度冲刺或极限测试。
    """
    return {
        "name": "首周计划草案",
        "status": "draft_requires_confirmation",
...
def _safety_notes(risks: set[str]) -> list[str]:
    ...
    安全说明始终提醒输出只是草案，并把高风险情况交给后续确认和 Java 侧规则处理。
    """
    notes = [
...
def _detect_risks(planning_input: Mapping[str, Any]) -> set[str]:
    """用关键词检测当前 Mock 草案需要保守处理的风险类别。

    这里不是医学诊断，只是为了测试和本地演示提供确定性的分支行为。
    """
